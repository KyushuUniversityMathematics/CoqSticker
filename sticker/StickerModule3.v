...
Definition onestep{symbol:finType}{rho:Rho symbol}
(A:seq (@domino symbol rho))(D:seq (@domino symbol rho*@domino symbol rho)):=
filter_option[seq mu' a d|a<-A,d<-D].
...
Structure sticker{symbol:finType}{rho:Rho symbol}:= Sticker{
  ...
  startP : all(fun p => match p with LR _ _ _ => true | _ => false end)start
}.
