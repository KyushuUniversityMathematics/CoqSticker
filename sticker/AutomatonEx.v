...
HB.instance Definition Z2_eqType := hasDecEq.Build Z2 eq_Z2P.
(* Definition ab_eqMixin := EqMixin eq_abP.
Canonical ab_eqType := Eval hnf in EqType _ ab_eqMixin. *)
HB.instance Definition ab_eqType := hasDecEq.Build ab eq_abP.
...
HB.instance Definition Z2_countType := isCountable.Build _ Z2_count_spec.
HB.instance Definition ab_countType := isCountable.Build _ ab_count_spec.
...
(*finType属性を付与*)
HB.instance Definition Z2_finType := isFinite.Build Z2 enum_Z2P.
HB.instance Definition ab_finType := isFinite.Build ab enum_abP.
...
Definition p1 := Automaton Z2 ab zero p1_d [set one].
